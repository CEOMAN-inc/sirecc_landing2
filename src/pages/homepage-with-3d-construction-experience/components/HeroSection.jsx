...
const WHATSAPP_NUMBER = "573225107655";
...
const HeroSection = () => {
  ...
  // >>> Fuente de eventos para R3F (así Canvas lee el mouse aunque tenga pointer-events:none)
  const [evtSrc, setEvtSrc] = useState(null);
  useEffect(() => {
    if (heroRef.current) setEvtSrc(heroRef.current);
  }, []);
  ...
  return (
    <section
      ref={heroRef}
      className="relative h-screen overflow-hidden bg-[var(--base-bg)]"
      style={{ ["--base-bg"]: BASE_BG }}
    >
      {/* CAPA 1: fondo y efectos */}
      <div className="absolute inset-0 z-0 pointer-events-none">
        ...
        <div
          ...
          style={{
            background:
              "radial-gradient(ellipse at center, rgba(0,0,0,0.28), rgba(0,0,0,0.52) 60%, rgba(0,0,0,0.6))",
          }}
        />
        {/* Spotlight derecha */}
        <div
          ...
          style={{
            opacity: 0.6,
            background:
              "radial-gradient(700px 700px at 80% 40%, rgba(255,255,255,0.22), rgba(255,255,255,0) 60%)",
          }}
        />
        ...
        <div
          ...
          style={{
            background:
              "linear-gradient(90deg, rgba(0,0,0,.45) 0%, rgba(0,0,0,.35) 35%, rgba(0,0,0,0) 70%)",
          }}
        />
      </div>

      {/* CAPA 2: blur suave encima del fondo (z-5 para dejar pasar la esfera) */}
      <div
        ...
        style={{
          ...
          WebkitMaskImage:
            "linear-gradient(to right, rgba(0,0,0,1) 0%, rgba(0,0,0,.85) 30%, rgba(0,0,0,0) 65%)",
          maskImage:
            "linear-gradient(to right, rgba(0,0,0,1) 0%, rgba(0,0,0,.85) 30%, rgba(0,0,0,0) 65%)",
        }}
      />

      {/* CAPA 2.5: Esfera 3D a la derecha (no ocupa todo) */}
      <div
        className="absolute inset-y-0 right-0 w-[52%] md:w-[48%] z-10"
        style={{
      ...
      >
        <Suspense fallback={null}>
          <Canvas
            // 🔑 Hace que R3F escuche los eventos del mouse desde el <section>, no desde la canvas
            eventSource={evtSrc}
            eventPrefix="client"
            ...
            onCreated={({ gl }) => {
              gl.setClearColor(0x000000, 0); // transparencia real
            }}
          >
            ...
            <SphereOfSpheres
              ...
              autoRotate={0.14}
              mouseIntensity={0.68}
              position={[2.5, 0.5, 1]}   // esfera desplazada hacia la derecha dentro del canvas
              // colores
              baseInnerColor={NAVY}
              ...
              materialOpacity={0}
              pointSize={5}
              // si usas explosión por clic/hover, tu lógica sigue igual
            />
          </Canvas>
        </Suspense>
      </div>
      ...
      <div className="relative z-20 h-full flex pt-40 md:pt-21 pointer-events-none">
        <div className="max-w-7xl mx-auto px-6 lg:px-8 w-full">
          <div ref={contentRef} className="max-w-4xl">
            ...
            >
              Soluciones integrales en ingeniería civil: obra nueva, remodelaciones y mantenimiento
              locativo con cumplimiento normativo y mano de obra calificada.
            </motion.p>
              ...
              <div className="relative group">
                ...
                >
                  <span className="flex items-center gap-2">
                    <svg viewBox="0 0 32 32" className="w-[18px] h-[18px] shrink-0" aria-hidden="true" style={{ fill: "#25D366" }}>
      ...
      {/* Feather inferior para fundir con la siguiente sección */}
      <div
        className="pointer-events-none absolute bottom-0 left-0 right-0 h-16 md:h-24 z-[15]"
        style={{
